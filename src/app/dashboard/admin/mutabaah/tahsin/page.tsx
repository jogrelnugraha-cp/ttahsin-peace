  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: recordData } = await supabase
        .from('daily_records')
        .select('id, date, attendance, tahsin_material, grade, notes')
        .eq('student_id', studentId)
        .in('program', ['Tahsin', 'Keduanya'])
  ...
  return (
    <div className="min-h-screen bg-white p-8 font-serif text-slate-900 leading-normal selection:bg-slate-200">
      ...
      <div className="no-print mb-6 flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-200">
        ...
        <button
          onClick={handlePrint}
          className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg font-bold text-sm shadow-sm transition"
        >
          🖨️ Cetak Sekarang
        </button>
      </div>
      ...
      <div className="text-center space-y-2 border-b-2 border-slate-800 pb-5">
        <h1 className="text-3xl font-extrabold tracking-wide uppercase">MUTABA&apos;AH AKADEMIK TAHSIN</h1>
      ...
      <div className="my-6 grid grid-cols-2 gap-4 text-sm border border-slate-300 p-4 rounded-lg bg-slate-50/50">
        ...
        <div>
          <span className="font-semibold text-slate-500">Tingkat Tahsin Saat Ini:</span>
          <p className="font-bold text-emerald-800 text-base">{student.tahsin_level || 'Jilid 1'}</p>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse border border-slate-300 text-xs">
          <thead>
            <tr className="bg-slate-100 border-b border-slate-300 text-slate-700 font-bold uppercase">
              ...
              <th className="border border-slate-300 p-2.5">Materi Tahsin</th>
            ...
            {records.length === 0 ? (
              <tr>
                <td colSpan={5} className="border border-slate-300 p-6 text-center text-slate-400 italic">
                  Belum ada riwayat perkembangan belajar Tahsin untuk santri ini.
                </td>
              </tr>
            ) : (
              records.map((rec) => (
